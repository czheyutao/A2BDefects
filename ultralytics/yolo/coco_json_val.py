...
# 模型名称
model_names = ["test-11l-seg-no","test-v8l-seg-no","test-v9c-seg-no","test-v10l-seg-no",]  # 这里可以替换为你的模型名称
for model_name in model_names:
    types = [""]
    for type in types:
        # 保存到 CSV 文件
        output_path = f"/data/hyt/SAM/box_results/{model_name}{type}.csv"
        ...
        # +++ 新增过滤逻辑 +++
        # 加载原始检测结果文件
        dt_path = f"/data/hyt/yolo/yoloruns/WZ/{model_name}/predictions.json"
        ...
        print(df)
        # 仅打印小、中、大的AP
        print(df[['AP-Small', 'AP-Medium', 'AP-Large']])
        # print(results[0]["AP-Small"], results[0]["AP-Medium"], results[0]["AP-Large"])