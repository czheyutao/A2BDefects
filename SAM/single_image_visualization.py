...
import json
def visualize_single_image(coco, img_id, save_dir, model_name):
    """
    可视化单张图片的SAM预测结果并保存
    参数:
        coco: COCO对象
...
if __name__ == "__main__":
    # 数据集路径
    data_root = "/data/hyt/mmdetection/mcrack1300"
    data_type = "train"
    ...
    # selected_img_ids.append(50)
    # selected_img_ids.append(656)
    # selected_img_ids.append(425)
    selected_img_ids = [743, 744, 745, 746, 747, 748, 749, 750, 751, 752, 753, 754, 755, 756, 757, 758]
    # 可视化dt
    # 遍历五张图片进行可视化
    for img_id in selected_img_ids:
        save_dir = f"/data/hyt/SAM/mcrack/{img_id}"
        os.makedirs(save_dir, exist_ok=True)
    ...
    #     # 遍历五张图片进行可视化
    #     for img_id in selected_img_ids:
    #         save_dir = f"/data/hyt/SAM/single_image_results/{img_id}"
    #         os.makedirs(save_dir, exist_ok=True)
