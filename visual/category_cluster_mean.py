...
class BboxSizeAnalyzer:
    ...
    def cluster_class_areas(self, n_clusters=3):
        """对每个类别单独进行分位数划分，类别3跳过，类别1和2使用2个聚类"""
        for category_id, areas in self.class_areas.items():
            ...
            percentiles = []
            for i in range(1, current_n_clusters):
                percentile = (i / current_n_clusters) * 100
                percentiles.append(percentile)

            thresholds = np.percentile(areas_array, percentiles).tolist()
